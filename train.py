if __name__ == '__main__':
    ...
    # 使用GPU
    netG_A2B.cuda(device)
    netG_B2A.cuda(device)
    netD_A.cuda(device)
    netD_B.cuda(device)
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            pred_fake = netD_B(fake_B).to(device)
            loss_GAN_A2B = criterion_GAN(pred_fake.to(device), target_real.to(device))

            fake_A = netG_B2A(real_B).to(device)
            pred_fake = netD_A(fake_A).to(device)
            loss_GAN_B2A = criterion_GAN(pred_fake.to(device), target_real.to(device))
            ...
            loss_D_real = criterion_GAN(pred_real.to(device), target_real.to(device))

            # Fake loss
            fake_A = fake_A_buffer.push_and_pop(fake_A)
            pred_fake = netD_A(fake_A.detach())
            loss_D_fake = criterion_GAN(pred_fake.to(device), target_fake.to(device))
